...
internal class PinchDetector(
  ...
  override fun onScale(detector: ScaleGestureDetector): Boolean {
    ...
    val newAbsFocusPoint =
      containerPointToContentPoint(PointF(-detector.focusX, -detector.focusY))
  ...
  private fun handleOnScaleEnd() {
    ...
    @ZoomApi.RealZoom val newZoom = zoomManager.checkBounds(
      matrixController.zoom,
      allowOverZoom = false,
    )
  ...
  private fun computeZoomPivot(fixPan: AbsolutePoint): PointF {
    if (matrixController.zoom <= 1F) {
      ...
      val center =
        AbsolutePoint(
          -matrixController.contentWidth / 2F,
          -matrixController.contentHeight / 2F,
        )

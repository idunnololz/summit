...
data class ScaledPoint(
  ...
  operator fun minus(scaledPoint: ScaledPoint): ScaledPoint =
    ScaledPoint(this.x - scaledPoint.x, this.y - scaledPoint.y)
   ...
   */
  operator fun unaryMinus(): ScaledPoint = ScaledPoint(-this.x, -this.y)
  ...
  operator fun times(factor: Number): ScaledPoint =
    ScaledPoint(factor.toFloat() * this.x, factor.toFloat() * this.y)

...
object BetterLinkify {
   ...
   */
  private val IANA_TOP_LEVEL_DOMAINS: String = ("(?:"
   ...
   */
  private val LABEL_CHAR: String = "a-zA-Z0-9" + UCS_CHAR

  /**
   * Valid characters for IRI TLD defined in RFC 3987.
   */
  private val TLD_CHAR: String = "a-zA-Z" + UCS_CHAR

  /**
   * RFC 1035 Section 2.3.4 limits the labels to a maximum 63 octets.
   */
  private val IRI_LABEL: String =
    "[" + LABEL_CHAR + "](?:[" + LABEL_CHAR + "_\\-]{0,61}[" + LABEL_CHAR + "]){0,1}"

  /**
   * RFC 3492 references RFC 1034 and limits Punycode algorithm output to 63 characters.
   */
  private const val PUNYCODE_TLD: String = "xn\\-\\-[\\w\\-]{0,58}\\w"

  private val TLD: String = "(" + PUNYCODE_TLD + "|" + "[" + TLD_CHAR + "]{2,63}" + ")"

  private val HOST_NAME: String = "(" + IRI_LABEL + "\\.)+" + TLD

  private const val PROTOCOL: String = "(?i:http|https|rtsp)://"

  /* A word boundary or end of input.  This is to stop foo.sure from matching as foo.su */
  private const val WORD_BOUNDARY: String = "(?:\\b|$|^)"

  private val USER_INFO: String = ("(?:[a-zA-Z0-9\\$\\-\\_\\.\\+\\!\\*\\'\\(\\)"
    + "\\,\\;\\?\\&\\=]|(?:\\%[a-fA-F0-9]{2})){1,64}(?:\\:(?:[a-zA-Z0-9\\$\\-\\_"
    + "\\.\\+\\!\\*\\'\\(\\)\\,\\;\\?\\&\\=]|(?:\\%[a-fA-F0-9]{2})){1,25})?\\@")

  private const val PORT_NUMBER: String = "\\:\\d{1,5}"

  private val PATH_AND_QUERY: String = ("[/\\?](?:(?:[" + LABEL_CHAR
  ...
  private val STRICT_DOMAIN_NAME
    : Pattern = Pattern.compile("(?:" + STRICT_HOST_NAME + "|" + IP_ADDRESS + ")")

  /**
   * Regular expression that matches domain names without a TLD
   */
  private val RELAXED_DOMAIN_NAME: String =
   ...
   */
  private val WEB_URL_WITHOUT_PROTOCOL: String = ("("
    + WORD_BOUNDARY
    + "(?<!:\\/\\/)"
    + "("
    + "(?:" + STRICT_DOMAIN_NAME + ")"
   ...
   *
   */
  private val AUTOLINK_WEB_URL: Pattern = Pattern.compile(
   ...
   * defined in RFC5321: #&~!^`{}/=$*?|
   */
  private val EMAIL_CHAR: String = LABEL_CHAR + "\\+\\-_%'"
   ...
   */
  private val EMAIL_ADDRESS_LOCAL_PART: String =
   ...
   */
  private val EMAIL_ADDRESS_DOMAIN: String = "(?=.{1,255}(?:\\s|$|^))" + HOST_NAME

  /**
   * Regular expression pattern to match email addresses. It excludes double quoted local parts
   * and the special characters #&~!^`{}/=$*?| that are included in RFC5321.
   */
  private val AUTOLINK_EMAIL_ADDRESS: Pattern = Pattern.compile(
    "(" + WORD_BOUNDARY +
  ...
  private val COMPARATOR: java.util.Comparator<LinkSpec?> =
    Comparator { a: LinkSpec, b: LinkSpec ->
      if (a.start < b.start) {
        return@Comparator -1
      }
      if (a.start > b.start) {
  ...
  fun addLinks(text: Spannable, @LinkifyCompat.LinkifyMask mask: Int): Boolean {
    if (mask == 0) {
      return false
    }

    val old = text.getSpans<URLSpan?>(0, text.length, URLSpan::class.java)
    ...
    if ((mask and Linkify.EMAIL_ADDRESSES) != 0) {
      gatherLinks(
        links, text, AUTOLINK_EMAIL_ADDRESS,
        arrayOf<String>("mailto:"),
        null, null,
      )
    }
  ...
  private fun gatherLinks(
    ...
    while (m.find()) {
      ...
      if ((matchFilter == null || matchFilter.acceptMatch(s, start, end)) && match != null) {
        ...
        spec.start = start
        spec.end = start + cleanedMatch.length
  ...
  private fun makeUrl(
    ...
    for (prefix in prefixes) {
      if (url.regionMatches(0, prefix, 0, prefix.length, ignoreCase = true)) {
        hasPrefix = true

        // Fix capitalization if necessary
        if (!url.regionMatches(0, prefix, 0, prefix.length, ignoreCase = false)) {
  ...
  private fun pruneOverlaps(links: java.util.ArrayList<LinkSpec>, text: Spannable) {
    ...
    while (i < len - 1) {
      ...
      if ((a.start <= b.start) && (a.end > b.start)) {
        ...
        if (remove != -1) {
          val span = links.get(remove).frameworkAddedSpan
          if (span != null) {

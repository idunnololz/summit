...
sealed interface ImageViewerResult : Parcelable {
  @Parcelize
  data class ShareImage(val url: String) : ImageViewerResult

  @Parcelize
  data class CopyImage(val url: String) : ImageViewerResult

  @Parcelize
  data class OpenPage(val pageRef: PageRef) : ImageViewerResult

  @Parcelize
  data object ErrorCustomDownloadLocation : ImageViewerResult
}

fun ImageViewerResult.toIntent() =
  Intent().apply {
